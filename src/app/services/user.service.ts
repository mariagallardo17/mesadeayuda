...
export class UserService {
  ...
  // Usuarios predefinidos eliminados - ahora solo usamos la API
  ...
  private loadUsers(): void {
    ...
    this.http.get<User[]>(`${ApiConfig.API_BASE_URL}/api/users`)
      .pipe(
        catchError(error => {
  ...
  getUsers(): Observable<User[]> {
    // Recargar desde la API cada vez que se solicite
    this.loadUsers();
    return this.users$;
  }
  ...
  createUser(userData: CreateUserRequest): Observable<User> {
    ...
    return this.http.post<{message: string, user: User}>(`${ApiConfig.API_BASE_URL}/api/users`, requestData)
      .pipe(
        map(response => {
          // Agregar usuario a la lista local sin recargar toda la lista
          this.addUserToList(response.user);
  ...
  updateUser(id: number, userData: UpdateUserRequest): Observable<User> {
    ...
    return this.http.put<{message: string}>(`${ApiConfig.API_BASE_URL}/api/users/${id}`, requestData)
      .pipe(
        map(response => {
  ...
  deleteUser(id: number): Observable<boolean> {
    return this.http.delete<{message: string}>(`${ApiConfig.API_BASE_URL}/api/users/${id}`)
      .pipe(
        map(response => {
  ...
  resetPassword(id: number): Observable<string> {
    return this.http.post<{message: string, newPassword: string}>(`${ApiConfig.API_BASE_URL}/api/users/${id}/reset-password`, {})
      .pipe(
        map(response => {
