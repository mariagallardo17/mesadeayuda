...
class ReportRoutes
{
    ...
    public function getReportsSummary()
    {
        ...
        try {
            ...
            // 3.5. Obtener distribución de evaluaciones (incluye evaluaciones normales y de reapertura)
            ...
            try {
                ...
                $evalQuery = $fechaInicio && $fechaFin
                    ? "SELECT calificacion, COUNT(*) as cantidad
                       FROM (
                         SELECT calificacion, fecha_evaluacion
                         FROM evaluaciones
                ...
                $paramsEval = $fechaInicio && $fechaFin ? [$fechaInicio, $fechaFin, $fechaInicio, $fechaFin] : [];
                $stmtEval = $this->db->query($evalQuery, $paramsEval);
                ...
                error_log('⚠️ Tabla evaluaciones_reaperturas no disponible, usando fallback para distribución de evaluaciones: ' . $e->getMessage());
                try {
                    $evalQuery = $fechaInicio && $fechaFin
                        ? "SELECT calificacion, COUNT(*) as cantidad
                           FROM evaluaciones
                    ...
                    $paramsEval = $fechaInicio && $fechaFin ? [$fechaInicio, $fechaFin] : [];
                    $stmtEval = $this->db->query($evalQuery, $paramsEval);
                    $evalRaw = $stmtEval->fetchAll();
                    ...
                    error_log('⚠️ Error obteniendo distribución de evaluaciones (fallback): ' . $e2->getMessage());
