...
try {
    ...
    if (!empty($smtpFrom)) {
        ...
        // Si el email del remitente no coincide con SMTP_USER, usar SMTP_USER
        if (!empty($smtpUser) && strtolower($fromEmail) !== strtolower($smtpUser)) {
            echo "⚠️  ADVERTENCIA: SMTP_FROM ($fromEmail) no coincide con SMTP_USER ($smtpUser)\n";
            echo "   Usando SMTP_USER como remitente para evitar problemas con Gmail.\n\n";
