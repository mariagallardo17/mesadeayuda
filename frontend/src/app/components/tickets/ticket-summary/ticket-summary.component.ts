...
export class TicketSummaryComponent implements OnInit, OnDestroy {
  ...
  calculateStats(): void {
    ...
    this.tickets.forEach(ticket => {
      ...
      // Si es reabierto, contarlo en reabiertos PERO también clasificarlo por su estado actual
      if (esReabierto) {
        this.stats.reabiertos++;
        ticket.mostrarEstadoReabierto = true;
        // También agregarlo a reabiertos para que aparezca en esa sección
        this.agregarTicketAEstado('reabierto', ticket);
      }

      // Clasificar por estado ACTUAL (tanto reabiertos como no reabiertos)
      switch (estadoLower) {
        case 'pendiente':
        case 'abierto':
        ...
        case 're-abierto':
        case 're_abierto':
          // Solo contar si NO es reabierto (para evitar doble conteo)
          if (!esReabierto) {
            this.stats.reabiertos++;
            this.agregarTicketAEstado('reabierto', ticket);
          }
