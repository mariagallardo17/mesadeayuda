    ...
    this.ticketService.getMyTickets(1, 10000).pipe(
      ...
      next: (response) => {
        ...
        if (response && response.tickets && response.pagination) {
          // Incluir todos los tickets: tickets normales y tickets reabiertos
          // Los tickets reabiertos mostrarán su estado anterior (Cerrado) mediante estado_reapertura
          // PERO permitir tickets "Finalizados" para que los empleados puedan evaluarlos
          // IMPORTANTE: Para técnicos, incluir TODOS los tickets que vienen del backend
          // El backend ya filtra correctamente y los tickets reabiertos muestran estado "Cerrado"
          // No filtrar en el frontend para técnicos - confiar en el backend
          const currentUser = this.authService.getCurrentUser();
          const esTecnico = currentUser?.rol === 'tecnico' || currentUser?.rol === 'administrador';
          
          if (esTecnico) {
            // Para técnicos: incluir TODOS los tickets que vienen del backend
            // El backend ya filtra correctamente y los tickets reabiertos se muestran con estado "Cerrado"
            this.tickets = response.tickets;
          } else {
            // Para empleados: aplicar filtro para excluir tickets cerrados (excepto reabiertos y finalizados)
            this.tickets = response.tickets.filter(ticket => {
              const estadoLower = (ticket.estado || '').toLowerCase().trim();
              
              // Incluir tickets reabiertos (mostrarán estado anterior "Cerrado")
              // El backend ya configuró el estado correcto usando estado_reapertura
              ...
              // PERO permitir tickets reabiertos (mostrarán estado anterior "Cerrado")
              ...
              const esReabierto = ticket.reapertura && ticket.reapertura !== null;
              ...
              // Permitir tickets reabiertos (mostrarán estado anterior "Cerrado")
              if (esReabierto) {
                return true;
              }
