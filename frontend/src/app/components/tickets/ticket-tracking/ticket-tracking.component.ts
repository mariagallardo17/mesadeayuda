...
export class TicketTrackingComponent implements OnInit, OnDestroy {
  ...
  loadTickets(): void {
    ...
    this.ticketService.getMyTickets(1, 10000, false, true).pipe(
      ...
      next: (response) => {
        ...
        // Excluir tickets reabiertos de "Seguimiento de Tickets"
        // Los tickets reabiertos tienen su propia sección "Tickets Reabiertos"
        const visibles = tickets.filter((ticket: any) => {
          if (!ticket || typeof ticket !== 'object') return false;
          // Excluir tickets reabiertos (tienen reapertura_id o reapertura)
          return !ticket.reapertura_id && !ticket.reapertura;
        });
        
        console.log('✅ Tickets visibles (sin reabiertos):', visibles.length);
      ...
      next: (response) => {
        ...
        const updateList = (list: Ticket[]) => {
          ...
          if (index !== -1) {
            ...
            // NO eliminar el ticket de la lista - debe permanecer en seguimiento mostrando estado original (Cerrado)
            // El backend ya está configurado para mostrar estado_reapertura en lugar del estado actual
          }
        };

        if (this.selectedTicket?.id === ticketId) {
          applyResponse(this.selectedTicket);
          // NO cerrar el modal automáticamente - el usuario puede seguir viendo el ticket
        }

        updateList(this.tickets);
        updateList(this.filteredTickets);

        // Recargar la lista para obtener el estado correcto desde el backend
        this.loadTickets();

        this.notificationService.addNotification({
          type: 'info',
          title: 'Ticket Reabierto',
          message: `El ticket #${ticketId} fue reabierto correctamente. Puedes verlo en "Tickets Reabiertos" con estado Pendiente.`,
          ticketId: ticketId.toString(),
          actionUrl: `/tickets/reopened`
        });

        // NO navegar automáticamente - dejar que el usuario decida si quiere ir a tickets reabiertos
      },
      error: (error) => {
        this.isSubmittingReopen = false;
