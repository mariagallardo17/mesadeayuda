...
export class ManageUsersComponent implements OnInit, OnDestroy {
  ...
  canDeleteUser(user: User): boolean {
    ...
    // No se pueden eliminar administradores ni técnicos
    return userRole !== 'administrador' && userRole !== 'tecnico';
  }
  ...
  deleteUser(user: User): void {
    ...
    // Verificar si el usuario es administrador o técnico
    const userRole = user.rol?.toLowerCase();
    if (userRole === 'administrador') {
      this.errorMessage = 'No se puede eliminar un usuario administrador. Los administradores son super usuarios y no pueden ser eliminados.';
      this.showErrorModal = true;
      return;
    }

    if (userRole === 'tecnico') {
      this.errorMessage = 'No se pueden eliminar usuarios técnicos. Considera desactivarlos en lugar de eliminarlos.';
      this.showErrorModal = true;
      return;
    }
    ...
    this.confirmAction = () => {
      ...
      this.userService.deleteUser(user.id!)
        ...
        .subscribe({
          ...
          error: (error) => {
            this.errorMessage = error.error?.error || error.message || 'Error al eliminar el usuario';
            this.showErrorModal = true;
            this.isLoading = false;
          }
        });
