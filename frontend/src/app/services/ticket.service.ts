...
export class TicketService {
  ...
  getMyTickets(page: number = 1, limit: number = 10, finalizadosSinEvaluar: boolean = false, includeClosed: boolean = false): Observable<{tickets: Ticket[], pagination: any}> {
    // Obtener token de autenticación
    const token = this.authService.getToken();
    let headers = new HttpHeaders();
    ...
    // Si se solicita incluir tickets cerrados (para el resumen de tickets)
    if (includeClosed) {
