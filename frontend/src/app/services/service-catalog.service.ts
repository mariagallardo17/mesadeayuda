...
export class ServiceCatalogService {
  ...
  createService(serviceData: CreateServiceRequest): Observable<ServiceCatalog> {
    return this.http.post<{message?: string, service: ServiceCatalog}>(`${ApiConfig.API_BASE_URL}/services`, serviceData, {
      observe: 'body',
      responseType: 'json'
    })
      .pipe(
        map(response => {
          console.log('✅ Respuesta del backend al crear servicio:', response);
          // El backend devuelve {message: '...', service: {...}}
          if (response && response.service) {
            return response.service;
          }
          // Si no hay service, intentar usar la respuesta completa como servicio
          return response as any as ServiceCatalog;
        }),
